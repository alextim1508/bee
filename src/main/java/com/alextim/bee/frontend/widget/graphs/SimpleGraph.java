...
import static com.alextim.bee.context.Context.DATE_FORMAT;
...
public class SimpleGraph extends AbstractGraph {
    ...
    private final String promptFormat = "%s Номер: %s Счет: %.1f Время: %s";

    public void addPoint(int i, long x, double y) {
        scoresDataSet.add(x, y);
        scoresDataSet.addDataLabel(i, String.format(promptFormat, title.get(), i, y, DATE_FORMAT.format(new Date(x))));
    }
