...
public final class DetectorCodes {
    ...
    @AllArgsConstructor
    public enum Event {
        RESTART((byte) 0x01, "Событие запуска/перезапуска контроллера блока детектирования"),
        STATE((byte) 0x10, "Событие текущего состояния блока детектирования"),
        INTERNAL_DATA((byte) 0x20, "Событие внутренних данных блока детектирования");
    ...
    @AllArgsConstructor
    public enum Command {
        RESTART((byte) 0x10, "Команда перезапуска контроллера блока детектирования"),
        GET_VERSION((byte) 0x11, "Команда чтения версии прошивки блока детектирования"),
        ...
        SET_IP_ADDR((byte) 0xC0, "Команда смены IP адреса блока детектирования");
