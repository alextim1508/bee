...
@Slf4j
public class DetectorClientInitializer extends DetectorClientAbstract {

    private final String IP;
    private final int port;
    private final int rcvBufSize;

    protected UpdDetectorTransfer transfer;

    protected AtomicBoolean isConnected = new AtomicBoolean(false);

    public DetectorClientInitializer(String IP, int port, int rcvBufSize, LinkedBlockingQueue<DetectorMsg> queue) {
        super(queue);
        this.IP = IP;
        this.port = port;
    ...
    @Override
    public void connect() {
        transfer = new UpdDetectorTransfer("DetectorTransfer", (bytes) -> {
